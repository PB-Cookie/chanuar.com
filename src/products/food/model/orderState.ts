...
export function initialOrderWorkflow(data: OrderRouteData): OrderWorkflow {
  if (data.order) return { active: data.menu, savedOrder: data.order, credential: data.credential, editing: false };
...
export function orderWorkflowReducer(state: OrderWorkflow, action: OrderAction): OrderWorkflow {
  switch (action.type) {
    ...
    case 'saved': return { ...state, credential: action.credential, savedOrder: action.order, editing: false };
