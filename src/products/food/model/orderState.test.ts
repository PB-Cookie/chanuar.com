...
const menu = { cycle: { id: 'cycle-1', status: 'open', openedAt: '' }, restaurant: { id: 'r', name: 'R', description: '', imageUrl: null, sourceUrl: null, availableItems: 1, openingHours: [] }, menuItems: [] } satisfies ActiveMenu;
const credential = { cycleId: 'cycle-1', orderId: 'order-1', token: 'secret' } satisfies Credential;
const order = { id: 'order-1', cycleId: 'cycle-1', cycleStatus: 'open', displayName: 'Ana', note: '', createdAt: '', updatedAt: '', totalCents: 0, restaurant: menu.restaurant, items: [] } satisfies FoodOrder;
...
describe('food order workflow reducer', () => {
  ...
  it('records credentials before confirmation is available', () => {
    ...
    expect(orderWorkflowReducer(state, { type: 'credential-recorded', credential })).toMatchObject({ credential, editing: true });
  });

  it('shows a submitted order and leaves edit mode', () => {
    const state = initialOrderWorkflow({ menu, credential: null, order: null });
    expect(orderWorkflowReducer(state, { type: 'saved', credential, order })).toMatchObject({ savedOrder: order, editing: false });
  });

  it('keeps the receipt when the cycle closes', () => {
    const closed = { ...order, cycleStatus: 'closed' };
    expect(orderWorkflowReducer(initialOrderWorkflow({ menu, credential, order }), { type: 'closed', order: closed })).toMatchObject({ savedOrder: closed, editing: false });
  });
