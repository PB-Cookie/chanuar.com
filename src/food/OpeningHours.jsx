...
export function OpeningHours({ openingHours, compact = false }) {
  ...
  if (!normalized.length) {
    return <div className={`food-hours-empty${compact ? ' food-hours-empty--compact' : ''}`}>Horario pendiente de confirmar</div>;
  }
        ...
        {WEEK_DAYS.map((day) => {
          ...
          return (
            <div className={day.id === today.day ? 'is-today' : ''} key={day.id}>
              <dt>{day.label}<span className="sr-only">{day.id === today.day ? ', hoy' : ''}</span></dt>
...
export function OpeningHoursForm({ restaurant, onSave, pending = false }) {
  ...
  return (
    <form className="food-hours-form" onSubmit={(event) => { event.preventDefault(); onSave(restaurant.id, schedule.filter((entry) => entry.periods.length)); }}>
          ...
          return (
            <fieldset className="food-hours-form__day" key={day.id}>
              ...
              <label className="food-hours-form__toggle">
                <input type="checkbox" checked={enabled} onChange={(event) => toggleDay(day.id, event.target.checked)} />
                <span>{day.label}</span>
              </label>
              {enabled ? (
                <div className="food-hours-form__periods">
                  {entry.periods.map((period, index) => (
                    <div className="food-hours-form__period" key={`${day.id}-${index}`}>
                      <label>
                        <span className="sr-only">{day.label}, apertura del tramo {index + 1}</span>
                        <input type="time" required value={period.open} onChange={(event) => updatePeriod(day.id, index, 'open', event.target.value)} />
                      </label>
                      <span aria-hidden="true">–</span>
                      <label>
                        <span className="sr-only">{day.label}, cierre del tramo {index + 1}</span>
                        <input type="time" required value={period.close} onChange={(event) => updatePeriod(day.id, index, 'close', event.target.value)} />
                      </label>
                      {entry.periods.length > 1 && (
                        <button type="button" className="food-hours-form__remove" onClick={() => removePeriod(day.id, index)} aria-label={`Eliminar tramo ${index + 1} del ${day.label.toLocaleLowerCase('es')}`}>×</button>
                      )}
                    </div>
                  ))}
                  {entry.periods.length < 4 && <button type="button" className="food-hours-form__add" onClick={() => addPeriod(day.id)}>+ Añadir tramo</button>}
                </div>
      ...
      <button className="food-button" type="submit" disabled={pending} aria-describedby={`${formId}-help`}>
        {pending ? 'Guardando…' : 'Guardar horario'}
      </button>
      <p className="food-help" id={`${formId}-help`}>Las horas se muestran en horario de Canarias.</p>
    </form>
  );
}
