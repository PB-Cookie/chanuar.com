...
export function aggregateItems(orders: AdminOrder[]) {
  const items = new Map<string, { name: string; quantity: number; totalCents: number; notes: string[] }>();
  for (const order of orders) {
    for (const item of order.items) {
      const current = items.get(item.menuItemId) ?? { name: item.name, quantity: 0, totalCents: 0, notes: [] };
...
export function cycleAmounts(cycle: AdminCycle, serviceFeeCents = cycle.serviceFeeCents) {
  return { subtotal: cycle.subtotalCents, fee: serviceFeeCents, total: cycle.subtotalCents + serviceFeeCents };
}
