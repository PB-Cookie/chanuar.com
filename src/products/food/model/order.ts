...
export function cartTotal(cart: Cart, menuItems: Pick<MenuItem, 'id' | 'priceCents'>[]) {
  ...
  return Object.entries(cart).reduce((total, [id, entry]) => total + (byId.get(id)?.priceCents ?? 0) * entry.quantity, 0);
}
...
type OrderLike = { items?: Array<Pick<OrderItem, 'menuItemId' | 'quantity' | 'note'> & Partial<OrderItem>> };

export function orderToCart(order: OrderLike | null, menuItems: Pick<MenuItem, 'id'>[] | null = null): Cart {
...
export function cartToPayload(cart: Cart): OrderItemPayload[] {
  return Object.entries(cart)
    ...
    .map(([menuItemId, entry]) => ({ menu_item_id: menuItemId, quantity: entry.quantity, note: entry.note.trim() || null }));
}

export function validateOrder({ displayName, orderNote, cart }: { displayName: string; orderNote: string; cart: Cart }) {
  ...
  if (!entries.length || entries.every((entry) => entry.quantity < 1)) return 'Añade al menos un plato al pedido.';
  if (entries.some((entry) => entry.quantity < 1 || entry.quantity > MAX_QUANTITY)) return `Cada plato debe tener entre 1 y ${MAX_QUANTITY} unidades.`;
  if (entries.some((entry) => entry.note.length > 240)) return 'Las notas de los platos no pueden superar 240 caracteres.';
  return null;
}
